...
contract LzTestHelper is OptionsHelper {
    ...
    function setUpEndpoints(
        ...
        return
            setUpEndpoints(
                InitEndpointParam(_endpointNum, 0),
    ...
    function setUpEndpoints(
        ...
        uint8 _endpointNum = _endpointParam.endpointNum +
            _endpointParam.endpointAltNum;
        ...
        for (uint32 eid = 1; eid <= _endpointNum; eid++) {
            ...
            for (uint32 remoteEid = 1; remoteEid <= _endpointNum; remoteEid++) {
                // wire priceFeed
                {
                    uint128 denominator = fixture
                        .priceFeed
                        .getPriceRatioDenominator();
                    ILayerZeroPriceFeed.UpdatePrice[]
                        memory prices = new ILayerZeroPriceFeed.UpdatePrice[](
                            1
                        );
                ...
                // wire executor
                {
                    IExecutor.DstConfigParam[]
                        memory executorConfigParams = new IExecutor.DstConfigParam[](
                            1
                        );
                ...
                {
                    IDVN.DstConfigParam[]
                        memory verifierConfigParams = new IDVN.DstConfigParam[](
                            1
                        );
                    ...
                    for (uint256 i = 0; i < fixture.requiredDVNs.length; i++) {
                        fixture.requiredDVNs[i].setDstConfig(
                            verifierConfigParams
                        );
                    }
                    for (uint256 i = 0; i < fixture.optionalDVNs.length; i++) {
                        fixture.optionalDVNs[i].setDstConfig(
                            verifierConfigParams
                        );
                    }
                }
                ...
                // wire sendUln/receiveUln - verifiers
                {
                    SetDefaultUlnConfigParam[]
                        memory ulnParams = new SetDefaultUlnConfigParam[](1);
                    address[] memory requiredVerifiers = new address[](
                        verifierConfig.requiredDVNCount
                    );
                    for (uint256 i = 0; i < requiredVerifiers.length; i++) {
                        requiredVerifiers[i] = address(fixture.requiredDVNs[i]);
                    }
                    address[] memory optionalVerifiers = new address[](
                        verifierConfig.optionalDVNCount
                    );
                    ...
                    UlnConfig memory ulnConfig = UlnConfig({
                        ...
                        optionalDVNThreshold: verifierConfig
                ...
                // wire sendUln - executor
                {
                    SetDefaultExecutorConfigParam[]
                        memory executorParams = new SetDefaultExecutorConfigParam[](
                            1
                        );
                    ...
                    executorParams[0] = SetDefaultExecutorConfigParam(
                        remoteEid,
                        executorConfig
                    );
    ...
    function _deployEndpoint(LzFixture memory _fixture, bool isAlt) internal {
        if (isAlt) {
            ...
            _fixture.endpoint = ILayerZeroEndpointV2Ext(
                LzUtil.deployEndpointV2(_fixture.eid, address(this))
            );
    ...
    function _deployUln302(LzFixture memory _fixture) internal {
        ...
        ReceiveUln302 receiveUln = new ReceiveUln302(
            address(_fixture.endpoint)
        );
    ...
    function _deployDVNs(
        LzFixture memory _fixture,
        InitVerifierParam memory _config
    ) internal {
        ...
        require(
            _fixture.optionalDVNThreshold <= _fixture.optionalDVNs.length,
            "invalid optional threshold"
        );
    }
    ...
    function _verifyOnMsgLib(bytes calldata _packetBytes) external {
        ILayerZeroEndpointV2Ext endpoint = ILayerZeroEndpointV2Ext(
            endpoints[_packetBytes.dstEid()]
        );
        ...
        if (major == 3) {
            ...
            DVN[] memory dvns = new DVN[](
                config.requiredDVNCount + config.optionalDVNCount
            );
    ...
    function _commitVerification(
        ...
        if (major == 3) {
            ...
            bytes memory packetHeader = _packetBytes.header();
            bytes32 payloadHash = keccak256(_packetBytes.payload());
            VerificationState state = dstUln.verifiable(
                packetHeader,
                payloadHash
            );
            ...
            SimpleMessageLibMock(payable(receiveLib)).validatePacket(
                _packetBytes
            );
        }
        return true;
    ...
    function _nativeDropAndExecute(
        bytes calldata _packetBytes
    ) external payable {
        ...
        if (
            ...
            (uint256 amount, bytes32 receiver) = _parseExecutorNativeDropOption(
                options
            );
    ...
    function _lzReceive(
...

        {
            ...
            if (nativeToken != address(0)) {
                AltFeeTokenMock(nativeToken).mint(
                    _packetBytes.receiverB20(),
                    value
                );
                value = 0;
        ...
        for (uint256 i = 0; i < entries.length; i++) {
            ...
            if (
                ...
                composeLookup[guid].push(
                    ComposeMessage(from, to, guid, index, message)
                );
            }
        }
    }
    ...
    function _lzCompose(bytes calldata _packetBytes) public payable {
        ...
        (
            ...
            uint256 gas,
            uint256 value
        ) = _parseExecutorLzComposeOption(options);
