...
contract OptionsHelper {
    UlnOptionsMock public ulnOptions = new UlnOptionsMock();

    function _parseExecutorLzReceiveOption(
        bytes memory _options
    ) internal view returns (uint256 gas, uint256 value) {
    ...
    function _executorOptionExists(
        ...
        (exist, ) = _getExecutorOptionByOptionType(
            _options,
            _executorOptionType
        );
    }
    ...
    function _getExecutorOptionByOptionType(
        ...
        while (cursor < executorOpts.length) {
            (uint8 optionType, bytes memory op, uint256 nextCursor) = this
                .nextExecutorOption(executorOpts, cursor);
    ...
    ) external pure returns (uint128 gas, uint128 value) {
        return ExecutorOptions.decodeLzReceiveOption(_option);
    }

    function decodeNativeDropOption(
        bytes calldata _option
    ) external pure returns (uint128 amount, bytes32 receiver) {
