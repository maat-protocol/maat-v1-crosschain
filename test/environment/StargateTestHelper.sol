...
import {IStargateFeeLib} from "@stargatefinance/stg-evm-v2/src/interfaces/IStargateFeeLib.sol";
import {StargateType, IStargate} from "@stargatefinance/stg-evm-v2/src/interfaces/IStargate.sol";
import {TargetCredit, TargetCreditBatch} from "@stargatefinance/stg-evm-v2/src/interfaces/ICreditMessaging.sol";
import {OFTTokenERC20} from "@stargatefinance/stg-evm-v2/src/utils/OFTTokenERC20.sol";
import {PoolToken} from "@stargatefinance/stg-evm-v2/src/mocks/PoolToken.sol";
import {StargatePool} from "@stargatefinance/stg-evm-v2/src/StargatePool.sol";
import {StargateOFT} from "@stargatefinance/stg-evm-v2/src/StargateOFT.sol";
import {StargatePoolNative} from "@stargatefinance/stg-evm-v2/src/StargatePoolNative.sol";
import {TokenMessaging} from "@stargatefinance/stg-evm-v2/src/messaging/TokenMessaging.sol";
import {CreditMessaging, CreditBatch, Credit} from "@stargatefinance/stg-evm-v2/src/messaging/CreditMessaging.sol";
import {StargateBase, Ticket} from "@stargatefinance/stg-evm-v2/src/StargateBase.sol";
import {TaxiCodec} from "@stargatefinance/stg-evm-v2/src/libs/TaxiCodec.sol";
import {BusCodec} from "@stargatefinance/stg-evm-v2/src/libs/BusCodec.sol";
import {FeeLibV1, FeeConfig} from "@stargatefinance/stg-evm-v2/src/feelibs/FeeLibV1.sol";
...
contract StargateTestHelper is LzTestHelper {
    ...
    mapping(uint32 => mapping(uint16 => StargateFixture))
        public stargateFixtures; // eid => assetId => Fixture
    ...
    mapping(uint32 => mapping(uint32 => Amount[]))
        public passengersAmountLdCache; // fromEid => toEid => passengers
    ...
    function setUpStargate(
        ...
        require(
            _poolNum > 0 || _nativePoolNum > 0,
            "StargateTestHelper: at least 1 pool or native pool"
        );
        ...
        for (uint8 i = 0; i < chainNum; i++) {
            ...
            );
            {
                ...
                eids[0] = eid;
                tokenMessaging.initializeBusQueueStorage(
                    eids,
                    0,
                    QUEUE_CAPACITY - 1
                );
            ...
            CreditMessaging creditMessaging = new CreditMessaging(
                address(lzFixtures[i].endpoint),
                address(this)
            );
            ...
            for (uint8 j = 0; j < _assetNum; j++) {
                ...
                if (i < _poolNum) {
                    ...
                    OFTTokenERC20 token = new OFTTokenERC20(
                        OT_NAME,
                        OT_SYMBOL,
                        OT_DECIMALS
                    );
                    tokenAddress = address(token);
                    StargateOFT sg = new StargateOFT(
                        tokenAddress,
                        6,
                        address(lzFixtures[i].endpoint),
                        address(this)
        ...
        for (uint256 i = 0; i < chainNum; i++) {
            ...
            for (uint256 j = 0; j < chainNum; j++) {
                ...
                tokenMessaging.setMaxNumPassengers(
                    dstEid,
                ...
                for (uint16 assetId = 1; assetId <= _assetNum; assetId++) {
                    StargateBase sg = StargateBase(
                        stargateFixtures[eid][assetId].stargate
                    );
                ...
                tokenMessaging.setGasLimit(
                    dstEid,
                    MIN_TOKEN_GAS,
                    NATIVE_DROP_GAS_LIMIT
                );
    ...
    function setEnforcedOptions(
        ...
        bytes memory enforcedOption = OptionsBuilder
            .newOptions()
            .addExecutorLzReceiveOption(MIN_TOKEN_GAS, 0);
    ...
    function mintAndAddLiquidity(
        ...
        require(
            sgType == StargateType.Pool,
            "StargateTestHelper: only pool can add liquidity"
        );
    ...
    function sendCredit(
        ...
        MessagingFee memory messagingFee = messaging.quoteSendCredits(
            _dstEid,
            _creditBatches
        );
        VM.deal(planner, messagingFee.nativeFee);
        VM.prank(planner);
        messaging.sendCredits{value: messagingFee.nativeFee}(
            _dstEid,
            _creditBatches
        );
    }
    ...
    function sendCredit(
        ...
        creditBatches[0] = TargetCreditBatch({
            assetId: assetId,
            credits: new TargetCredit[](1)
        });
        creditBatches[0].credits[0] = TargetCredit({
            srcEid: _srcEid,
            amount: _amount,
            minAmount: 0
        });
    ...
    function send(
        ...
        MessagingFee memory messagingFee = StargateBase(_stargate).quoteSend(
            _sendParam,
            false
        );
    ...
    function rideBus(
        ...
        (, , OFTReceipt memory receipt) = StargateBase(_stargate).quoteOFT(
            sendParam
        );
        ...
        passengersAmountLdCache[_srcEid][_dstEid].push(
            Amount(assetId, sendParam.minAmountLD)
        );
    }
    ...
    function driveBus(
        ...
        (, , , uint16 length, uint72 nextTicketId) = tokenMessagingByEid[
            _fromEid
        ].busQueues(_toEid);
        ...
        for (
            uint72 index = nextTicketId;
            index < nextTicketId + passengerNum;
            index++
        ) {
            _passengers = abi.encodePacked(
                _passengers,
                passengersCache[_fromEid][_toEid][index]
            );
        }

        MessagingFee memory messagingFee = tokenMessagingByEid[_fromEid]
            .quoteDriveBus(_toEid, _passengers);
        VM.deal(_driver, messagingFee.nativeFee);
        VM.prank(_driver);
        tokenMessagingByEid[_fromEid].driveBus{value: messagingFee.nativeFee}(
            _toEid,
            _passengers
        );
    }

    function driveBusAndRelay(
        address _driver,
        uint32 _fromEid,
        uint32 _toEid
    ) internal {
    ...
    function redeemSend(
        ...
        StargatePool sg = StargatePool(_stargate);
        require(
            sg.stargateType() == StargateType.Pool,
            "StargateTestHelper: only pool can redeem"
        );
        SendParam memory sendParam = SendParam({
            dstEid: _toEid,
            to: addressToBytes32(_receiver),
    ...
    function _getCredit(
        address _stargate,
        uint32 _srcEid
    ) internal view returns (uint64 credit) {
        StargateBase sg = StargateBase(_stargate);
        credit = sg.paths(_srcEid);
    }

    function _safeGetStargateFixture(
        uint32 _eid,
        uint16 _assetId
    ...
    function _isBusFull(
        ...
        (, , , uint16 length, ) = tokenMessagingByEid[_fromEid].busQueues(
            _toEid
        );
